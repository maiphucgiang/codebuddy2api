...
async def _nonstream_adapted(url, headers, body, model_name, t0, rid, cred, *, anthropic=False,
    ...
    tool_namespaces = body.get("_tool_namespaces")
    ...
    async def fetch(routed, cred, headers, url):
        routed_body = {k: v for k, v in routed.items() if k != "_tool_namespaces"} if "_tool_namespaces" in routed else routed
    ...
    if policy.realtime:
        ...
        converter = (AnthropicStreamConverter(model=model_name) if anthropic else
                     ResponsesStreamConverter(model=model_name,
                                              ...
                                              tool_namespaces=tool_namespaces))
    sent = False
    upstream_body = {k: v for k, v in body.items() if k != "_tool_namespaces"} if "_tool_namespaces" in body else body
